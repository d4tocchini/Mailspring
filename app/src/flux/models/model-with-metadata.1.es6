...
const ModelWithMetadata_attributes = Object.create(Model.attributes)
ModelWithMetadata_attributes['pluginMetadata'] = Attributes.Collection({
  queryable: true,
...
module.exports = class ModelWithMetadata extends Model {

  static attributes = ModelWithMetadata_attributes
  ...
  metadataForPluginId(pluginId) {
    ...
    const str = JSON.stringify(metadata.value)
  ...
  directlyAttachMetadata(pluginId, metadataValue) {
    ...
    if (metadata.value.expiration) {
      metadata.value.expiration = Math.round(
        new Date(metadata.value.expiration).getTime() / 1000
        );
    }
    return this;
  }
  ...
  metadataObjectForPluginId(pluginId) {
    ...
    __pluginId = pluginId
    return this.pluginMetadata.find(_metadata_pluginid_equals);
  }
}
let __pluginId
function _metadata_pluginid_equals(metadata) {
  return metadata.pluginId === __pluginId
}
